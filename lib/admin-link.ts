...
export async function adminAccess(request:Request,key?:string):Promise<Response|null>{
 ...
 const key=new URLSearchParams(location.hash.slice(1)).get('key');history.replaceState(null,'','/admin');
 if(!key){document.getElementById('status').textContent='Buka link admin rahasia lengkap milikmu.'}
 else fetch('/api/admin-session',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key})}).then(r=>{if(!r.ok)throw Error();location.replace('/owner')}).catch(()=>document.getElementById('status').textContent='Link admin tidak valid atau belum diaktifkan.');
