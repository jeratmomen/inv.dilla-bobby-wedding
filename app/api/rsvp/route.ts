...
await db().prepare("INSERT INTO replies (id,guest_id,name,attendance,message,updated_at) VALUES (?,?,?,?,?,?)").bind(replyId,guestId,name,p.attendance,p.message.trim(),new Date().toISOString()).run();
